...
jsPsych.plugins["reward"] = (function() {
	...
	plugin.trial = function(display_element, trial) {
		...
		var gambleTimeout = trial.gamble_timeout;
	...
	//	var canvas = document.createElement("canvas");
		...
		function dispBalance(){
			var txt2disp = " Current Winnings: $"+parseFloat(currBalance).toFixed(2)
			ctx.font='24px Calibri';

			ctx.fillText(txt2disp,canvasWidth-200,32)
		...
		function showGhost(){
			...
			img1.onload=function(){
				ctx.drawImage(img1,(window.innerWidth/2-(imgSize/2)),(window.innerHeight/2-(imgSize/2)),imgSize,imgSize)
			}
			img1.src='jsPsych-6.0.1/stimuli/ghost.png';
		...
		function gambleResult(){
			console.log(trials2Lose[rewardTrialCounter])
			if (trials2Lose[rewardTrialCounter]==1 && choiceMade !== 0){

				choiceMade=0;
				showGhost();
				if (coherentDirectionArray[0]==0){
					console.log('drawing leftbox')
					drawLeftBox();
					clearRightBox();
				}else if (coherentDirectionArray[1]==0){
					console.log('drawing right box')
					drawRightBox();
					clearLeftBox();
				}
				var timeB4Disp=500;
			}else{var timeB4Disp=0;}
			...
			//	ctx.fillStyle='green';
				...
				var txt2disp='No bet placed! -$0.05';
			...
			window.setTimeout(function(){ctx.fillStyle=fillColor;ctx.fillText(txt2disp,(canvasWidth/2),(canvasHeight/2)+(apertureHeight/2));console.log(ctx.fillStyle)},timeB4Disp)
		}
		...
		function dispText(){
			...
			var txt2disp2='(Left or Right Arrow Key)';
