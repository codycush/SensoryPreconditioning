...
//console.log(layer1)
	...
	//	layer2.style.margin = 0;
		//layer2.style.padding = 0;
	...
	//	animateDotMotion();
		...
		function drawFix() {
		  //horizontal
		  ctx1.beginPath();
		  ctx1.lineWidth=fixationCrossThickness;
		  ctx1.moveTo(canvasWidth/2 - fixationCrossWidth, canvasHeight/2);
		  ctx1.lineTo(canvasWidth/2 + fixationCrossWidth, canvasHeight/2);
		  ctx1.strokeStyle = fixationCrossColor;
		  ctx1.stroke();
		  ctx1.closePath();
		  //vertical
		  ctx1.beginPath();
		  ctx1.lineWidth = fixationCrossThickness;
		  ctx1.moveTo(canvasWidth/2,canvasHeight/2 - fixationCrossHeight)
		  ctx1.lineTo(canvasWidth/2,canvasHeight/2 + fixationCrossHeight)
		  ctx1.strokeStyle = fixationCrossColor;
		  ctx1.stroke();
		  ctx1.closePath();
		}
      ...
      var dotArray = dotArray2d[currentSetArray[currentApertureNumber]];
	      ...
	      //Loop through the dots one by one and draw them
				for (var i = 0; i < nDots; i++) {
					dot = dotArray[i];
					ctx1.beginPath();
					ctx1.arc(dot.x, dot.y, dotRadius+1, 0, Math.PI * 2);
      ...
      var dotArray = dotArray2d[currentSetArray[currentApertureNumber]];
			...
			for (var i = 0; i < nDots; i++) {
				dot = dotArray[i];
				ctx1.beginPath();
				ctx1.arc(dot.x, dot.y, dotRadius, 0, Math.PI * 2);
				ctx1.fillStyle = dotColor;
				ctx1.fill();
			}
		   ...
		   /* if(fixationCross === true){
		      ...
		      ctx1.moveTo(canvasWidth/2 - fixationCrossWidth, canvasHeight/2);
		      ctx1.lineTo(canvasWidth/2 + fixationCrossWidth, canvasHeight/2);
		      ctx1.fillStyle = fixationCrossColor;
		      ctx1.stroke();
	      	...
	      	if(border === true){
	        	...
	        	if(apertureType === 1 || apertureType === 2){
	          		ctx1.lineWidth = borderThickness;
	          		ctx1.strokeStyle = borderColor;
	          		ctx1.beginPath();
	          		ctx1.ellipse(apertureCenterX, apertureCenterY, horizontalAxis+(borderThickness/2), verticalAxis+(borderThickness/2), 0, 0, Math.PI*2);
	          		ctx1.stroke();
	        	...
	        	if(apertureType === 3 || apertureType === 4){
	          		ctx1.lineWidth = borderThickness;
	          		ctx1.strokeStyle = borderColor;
	          		ctx1.strokeRect(apertureCenterX-horizontalAxis-(borderThickness/2), apertureCenterY-verticalAxis-(borderThickness/2), (horizontalAxis*2)+borderThickness, (verticalAxis*2)+borderThickness);
	        	}//End of if square or
      ...
      var dotArray = dotArray2d[currentSetArray[currentApertureNumber]];
		...
		function showImages(){
			...
			img1.onload=function(){
				ctx2.drawImage(img1,canvasWidth/2,canvasHeight/2)
			}
			img1.src=imageset.image1
			console.log(img1.src)
			var img2=new Image();
			img2.onload=function(){
				ctx2.drawImage(img2,canvasWidth/2-100,canvasHeight/2,100,100)
			}
			img2.src=imageset.image2
			var img3=new Image();
			img3.onload=function(){
				ctx2.drawImage(img3,canvasWidth/2,canvasHeight/2+100,100,100)
			}
			img3.src=imageset.image3
			var img4=new Image();
			img4.onload=function(){
				ctx2.drawImage(img4,canvasWidth/2,canvasHeight/2-100,100,100)
			}
